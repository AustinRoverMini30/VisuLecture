...
namespace VisuLecture.Components
{
    ...
    public partial class UploadController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        public async Task<IActionResult> Single(IFormFile file)
        {
            try
            {
                ...
                Console.WriteLine("Upload réussi !");

                var fileName = file.FileName;

                using (var stream = new FileStream(Path.Combine(environment.ContentRootPath, fileName), FileMode.Create))
                {
                    // Save the file
                    file.CopyTo(stream);
